...
export function createDeepQNetwork(h, w, numActions, otherStateLength) {
    ...
    const dLayer1 = tf.layers.dense({ units: 20, activation: 'softmax' }).apply(fLayer);
    
    const dLayer2 = tf.layers.dense({ units: otherStateLength, activation: 'relu' }).apply(input2);
    ...
    const dLayer3 = tf.layers.dense({ units: 20, activation: 'softmax' }).apply(concLayer);
    const dLayer4 = tf.layers.dense({ units: 10, activation: 'softmax' }).apply(dLayer3);
